...
export function createWeatherStore(): PollingStore<WeatherEvent> {
  const [data, setData] = createSignal<WeatherEvent[]>([]);
  ...
  const fetch = async () => {
    ...
    setError(null);
    try {
      const events = await fetchWeather();
      setData(events);
    } catch (err) {
  ...
  const subscribe = () => {
    fetch();
    intervalId = setInterval(fetch, API_CONFIG.weather.interval) as unknown as number;
  };
